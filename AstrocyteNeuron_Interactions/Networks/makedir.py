...
def smart_makedir(name_dir, level_up=0, trial=False):
    ...
    trail : bool, optional
        if "name_dir" already exists, create iterative sub-folder "trial-index" where new 
        data are loaded. Note that the name of such folder is fixed. Default=False
    ...
    # if path does not exist and I want the first trial
    if trial and not(os.path.exists(path)):
        os.makedirs(path+'/trial-1')
        return

    if os.path.exists(path):
        if trial:
            list_trial = [int(name_trial.split('-')[-1]) for name_trial in os.listdir(path)]
            os.makedirs(path+'/trial-'+str(len(list_trial)+1))
            return
        else:
